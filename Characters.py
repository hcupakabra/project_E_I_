...
class Player(Character):
    ...
    ANIMATION_STAY = [pygame.transform.scale(load_image('player/stay.png', colorkey=-1), (50, 50))]
...
class Enemy(Character):
    ...
    def __init__(self, hp, dmg, x, y, *groups):
        ...
        self.distance_to_player_x = 50
        self.distance_to_player_y = 10
    ...
    def update(self, player):
        ...
        if self.rect.x - player.rect.x > self.distance_to_player_x and self.rect.y - player.rect.y > self.distance_to_player_y:
            self.startAnimation(self.animation_IDLE)
        elif player.rect.x - self.rect.x <= self.distance_to_player_x and player.rect.y - self.rect.y <= self.distance_to_player_y:
            self.startAnimation(self.animation_LEFT)
            self.rect.x -= self.movement_speed
        elif (player.rect.x - player.w) + (self.rect.x - self.w) >= 1:
